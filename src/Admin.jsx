    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <a href="/#/" className="text-sm text-gray-400 hover:text-white">← View Site</a>
        </div>
...
function BrandsTab({ brands, fetchBrands, editingBrand, setEditingBrand, showMsg }) {
  ...
  useEffect(() => {
    if (editingBrand) setForm(editingBrand);
    ...
    <div>
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mb-8">
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">Logo Text (2-3 letters)</label>
      ...
      <div className="space-y-3">
        {brands.map(b => (
          <div key={b.id} className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex items-center justify-between">
            <div className="flex items-center space-x-4">
...
function ProductsTab({ brands, products, fetchProducts, editingProduct, setEditingProduct, showMsg }) {
  ...
  const saveProduct = async () => {
    ...
    if (editingProduct) {
      ...
      // Delete old specs and features, then re-insert
    ...
    if (validSpecs.length > 0) {
      await supabase.from('product_specs').insert(validSpecs.map(s => ({ product_id: productId, ...s })));
    }

    // Insert features
    const validFeatures = features.filter(f => f.trim());
    if (validFeatures.length > 0) {
      await supabase.from('product_features').insert(validFeatures.map((f, i) => ({ product_id: productId, feature: f, sort_order: i })));
    }
  ...
  const deleteProduct = async (id) => {
    if (!confirm('Delete this product and all its images/documents?')) return;
  ...
  return (
    <div>
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mb-8">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <select value={form.brand_id} onChange={e => setForm({...form, brand_id: e.target.value})} className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white">
          ...
          <div>
            ...
            <input value={form.model} onChange={e => setForm({...form, model: e.target.value})} placeholder="e.g. Z14" className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Product Name *</label>
            <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="e.g. Z14 Self-Service Juicer" className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Category</label>
            <input value={form.category} onChange={e => setForm({...form, category: e.target.value})} placeholder="e.g. Juicers" className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
          <div className="col-span-2">
            <label className="block text-sm text-gray-400 mb-1">Short Description</label>
            <input value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Brief one-liner..." className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
          <div className="col-span-2">
            <label className="block text-sm text-gray-400 mb-1">Full Description</label>
            <textarea value={form.full_description} onChange={e => setForm({...form, full_description: e.target.value})} rows={4} placeholder="Detailed product description..." className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
          <div className="col-span-2">
            <label className="block text-sm text-gray-400 mb-1">Video URL (YouTube embed URL)</label>
            <input value={form.video_url} onChange={e => setForm({...form, video_url: e.target.value})} placeholder="https://www.youtube.com/embed/..." className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          {specs.map((s, i) => (
            <div key={i} className="flex space-x-2 mb-2">
              <input value={s.spec_name} onChange={e => { const n = [...specs]; n[i].spec_name = e.target.value; setSpecs(n); }} placeholder="e.g. Dimensions" className="flex-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm" />
              <input value={s.spec_value} onChange={e => { const n = [...specs]; n[i].spec_value = e.target.value; setSpecs(n); }} placeholder='e.g. 17.9"W x 18.7"D x 30.5"H' className="flex-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm" />
              <button onClick={() => setSpecs(specs.filter((_, j) => j !== i))} className="px-3 py-2 bg-red-900/30 text-red-400 rounded-lg text-sm">×</button>
            </div>
          ))}
          <button onClick={() => setSpecs([...specs, { spec_name: '', spec_value: '' }])} className="text-sm text-red-500 hover:underline">+ Add Specification</button>
        </div>
        ...
        <div className="mb-4">
          ...
          {features.map((f, i) => (
            <div key={i} className="flex space-x-2 mb-2">
              <input value={f} onChange={e => { const n = [...features]; n[i] = e.target.value; setFeatures(n); }} placeholder="e.g. Self-service one-button operation" className="flex-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm" />
              <button onClick={() => setFeatures(features.filter((_, j) => j !== i))} className="px-3 py-2 bg-red-900/30 text-red-400 rounded-lg text-sm">×</button>
            </div>
          ))}
          <button onClick={() => setFeatures([...features, ''])} className="text-sm text-red-500 hover:underline">+ Add Feature</button>
        </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <select value={filterBrand} onChange={e => setFilterBrand(e.target.value)} className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm">
      ...
      <div className="space-y-3">
        {filtered.map(p => {
          ...
          const docCount = p.product_documents?.length || 0;
          return (
            <div key={p.id} className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-400">{b?.name} • {p.model} • {p.category}</div>
                  <div className="text-xs text-gray-500">{imgCount} images • {docCount} documents</div>
                </div>
              </div>
              <div className="flex space-x-2">
        ...
        {filtered.length === 0 && <p className="text-gray-500 text-center py-8">No products yet. Add your first product above.</p>}
      </div>
    </div>
  );
}
...
function ImagesTab({ products, fetchProducts, showMsg }) {
  ...
  const uploadImage = async (e) => {
    ...
    const { error: uploadError } = await supabase.storage.from('product-assets').upload(`images/${fileName}`, file);
    ...
    const { data: { publicUrl } } = supabase.storage.from('product-assets').getPublicUrl(`images/${fileName}`);

    await supabase.from('product_images').insert([{
      product_id: selectedProduct.id,
      image_url: publicUrl,
      alt_text: file.name,
      sort_order: selectedProduct.product_images?.length || 0
    }]);
  ...
  const uploadDocument = async (e) => {
    ...
    const { error: uploadError } = await supabase.storage.from('product-assets').upload(`documents/${fileName}`, file);
    ...
    const { data: { publicUrl } } = supabase.storage.from('product-assets').getPublicUrl(`documents/${fileName}`);
  ...
  return (
    <div>
      ...
      {/* Product Selector */}
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mb-8">
        <label className="block text-sm text-gray-400 mb-2">Select a product to manage its files</label>
        ...
        <>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mb-8">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              {(currentProduct.product_images || []).map(img => (
                <div key={img.id} className="relative bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
                  <img src={img.image_url} alt={img.alt_text} className="w-full h-48 object-cover" />
                  <div className="p-3">
                    <p className="text-xs text-gray-400 truncate">{img.alt_text}</p>
                    <button onClick={() => deleteImage(img.id, img.image_url)} className="mt-2 text-xs text-red-400 hover:underline">Delete</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6">
              ...
              {(currentProduct.product_documents || []).map(doc => (
                <div key={doc.id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-gray-400">{doc.file_type} • {doc.file_size}</div>
                  </div>
                  <div className="flex space-x-2">
                    <a href={doc.file_url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-gray-700 rounded-lg text-sm hover:bg-gray-600">View</a>
                    <button onClick={() => deleteDocument(doc.id)} className="px-4 py-2 bg-red-900/30 text-red-400 rounded-lg text-sm hover:bg-red-900/50">Delete</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              <label className="block bg-gray-800 border-2 border-dashed border-gray-600 rounded-xl p-6 text-center cursor-pointer hover:border-red-600 transition-colors">
...
function BlogTab({ showMsg }) {
  ...
  const savePost = async () => {
    ...
    if (editing) {
      await supabase.from('blog_posts').update({ ...form, published_at: form.published ? new Date().toISOString() : null }).eq('id', editing.id);
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="Post title" className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Category</label>
              <input value={form.category} onChange={e => setForm({...form, category: e.target.value})} placeholder="e.g. Maintenance, Trends" className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
            </div>
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Excerpt</label>
            <input value={form.excerpt} onChange={e => setForm({...form, excerpt: e.target.value})} placeholder="Brief summary..." className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Content</label>
            <textarea value={form.content} onChange={e => setForm({...form, content: e.target.value})} rows={8} placeholder="Full blog post content..." className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white" />
          </div>
          <label className="flex items-center space-x-2 cursor-pointer">
            <input type="checkbox" checked={form.published} onChange={e => setForm({...form, published: e.target.checked})} className="rounded" />
            <span className="text-gray-300">Published (visible on site)</span>
          </label>
          <div className="flex space-x-3">
        ...
        {posts.map(p => (
          <div key={p.id} className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-gray-400">{p.category} • {new Date(p.created_at).toLocaleDateString()}</div>
            </div>
            <div className="flex space-x-2">
